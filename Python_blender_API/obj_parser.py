...
class Vertex:
    ...
    def cross(self, other):
        return Vertex(-1, self.y * other.z - self.z * other.y, self.z * other.x - self.x * other.z,
...
class Model:
    ...
    def save_quality(self, output_path, indices: list[int]):
        f = open(output_path, 'w+')
        f_qlt = open(output_path.split('.')[0] + '.qlt', "w+")
        for ind, vert in enumerate(self.vertices):
            if ind in indices:
                color = [255, 0, 0]
                f_qlt.writelines("0.5\n")
            else:
                color = [255, 255, 255]
                f_qlt.writelines("1.0\n")
            f.writelines(f"{vert} {color[0]} {color[1]} {color[2]}\n")
        for face in self.faces:
            f.writelines(f"{face}\n")
    ...
    def colorize_vert_n_ppv(self, vertex_number, n):
        voisins = set()
        voisins.add(vertex_number)
        for i in tqdm(range(n)):
            vois = voisins.copy()
            for vertex_numb in vois:
                for face in self.faces:
                    face_vert = face.get_vertices()
                    if vertex_numb in face_vert:
                        voisins.update(face_vert)
        return voisins
        ...
        for i in range(n):
            ...
            for vertex_numb in vois:
                for face in self.faces:
                    ...
                    if vertex_numb in face_vert:
                        voisins.update(face_vert)
        for ind in reversed(sorted(voisins)):
            self.remove_vertex(ind)

    def remove_vert_dist(self, vert, dist):
        for voisin in reversed(sorted(self.getVoisins(vert, dist))):
            self.remove_vertex(voisin)
    ...
    def copy(self):
        ...
        # print("pirouette")
        # new_model.face_normals = [normal.copy() for normal in self.face_normals]
        # print("polochon")
        # for norm in self.vertex_normals:
        #     print(norm)
        #     print(type(norm))
        # new_model.vertex_normals = [self.vertex_normals[normal].copy() for normal in self.vertex_normals]
        # print("putois")
    ...
    def getDiagonal(self):
        (minx, maxx, miny, maxy, minz, maxz) = self.getBbox()
    ...
    def getVoisins(self, germe, dist):
        voisins = set()
        ...
        cont = True
        while cont:
            # print(f"VOISINS RESTANTS : {voisins_restants}")
            to_add = []
    ...
    #     to_delete = []
    #     for ind, face in enumerate(self.faces):
    #         vertices_list = face.get_vertices()
    #         for vert in vertices_list:
    #             if vertices_list.count(vert) > 1:
