...
class Voxel_Information
{
  public:
    ...
    bool is_free(double& occupancy) { return occupancy < p_unknown_lower_bound; }

    bool is_occupied(double& occupancy) { return occupancy > p_unknown_upper_bound; }
...
class View
{
  public:
    ...
    double global_function(int x) { return exp(-1.0 * x); }
                                       ...
                                       int space_id)
    {
        ...
        viewer->addLine<pcl::PointXYZ>(pcl::PointXYZ(init_pos(0), init_pos(1), init_pos(2)),
                                       ...
                                       0,
                                       0,
                                       "X" + to_string(space_id) + "-" + to_string(id));
        viewer->addLine<pcl::PointXYZ>(pcl::PointXYZ(init_pos(0), init_pos(1), init_pos(2)),
                                       pcl::PointXYZ(Y(0), Y(1), Y(2)),
                                       0,
                                       255,
                                       0,
                                       "Y" + to_string(space_id) + "-" + to_string(id));
        viewer->addLine<pcl::PointXYZ>(pcl::PointXYZ(init_pos(0), init_pos(1), init_pos(2)),
                                       pcl::PointXYZ(Z(0), Z(1), Z(2)),
                                       0,
                                       0,
                                       255,
                                       "Z" + to_string(space_id) + "-" + to_string(id));
...
class View_Space
{
  public:
               ...
               pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud)
    {
        ...
        { // Generate a viewpoint collection if it does not exist
            // Get Point Cloud BBX
            vector<Eigen::Vector3d> points;
