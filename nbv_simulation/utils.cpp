 ...
 * @param pixel The pixel coordinates of the 3d point
 * @param intrinsics The intrinsic parameters of the camera
 * @param point The 3d point to project in the image plane
 */
static void rs2_project_point_to_pixel(float pixel[2], const struct rs2_intrinsics *intrinsics, const float point[3]) {
    ...
    if (intrinsics->model == RS2_DISTORTION_FTHETA) {
        ...
        auto rd = (float) (1.0f / intrinsics->coeffs[0] * atan(2 * r * tan(intrinsics->coeffs[0] / 2.0f)));
        x *= rd / r;
        y *= rd / r;
    }
    if (intrinsics->model == RS2_DISTORTION_KANNALA_BRANDT4) {
        ...
        float series =
                1 + theta2 * (intrinsics->coeffs[0] +
                              theta2 * (intrinsics->coeffs[1] + theta2 * (intrinsics->coeffs[2] + theta2 * intrinsics->coeffs[3])));
...
static void rs2_deproject_pixel_to_point(float point[3],
                                         ...
                                         const float pixel[2],
                                         float depth) {
    assert(intrinsics->model != RS2_DISTORTION_MODIFIED_BROWN_CONRADY); // Cannot deproject from a forward-distorted image
    // assert(intrinsics->model != RS2_DISTORTION_BROWN_CONRADY); // Cannot deproject to a brown conrady model
    ...
    if (intrinsics->model == RS2_DISTORTION_KANNALA_BRANDT4) {
        ...
        float theta2 = rd * rd;
        for (int i = 0; i < 4; i++) {
            float f = theta * (1 + theta2 * (intrinsics->coeffs[0] +
                                             theta2 * (intrinsics->coeffs[1] +
                                                       theta2 * (intrinsics->coeffs[2] + theta2 * intrinsics->coeffs[3])))) -
                      rd;
            if (abs(f) < FLT_EPSILON) {
                break;
            }
            float df = 1 + theta2 * (3 * intrinsics->coeffs[0] +
                                     theta2 * (5 * intrinsics->coeffs[1] +
                                               theta2 * (7 * intrinsics->coeffs[2] + 9 * theta2 * intrinsics->coeffs[3])));
            theta -= f / df;
            theta2 = theta * theta;
        }
        float r = tan(theta);
    ...
    if (intrinsics->model == RS2_DISTORTION_FTHETA) {
        ...
        auto r = (float) (tan(intrinsics->coeffs[0] * rd) / atan(2 * tan(intrinsics->coeffs[0] / 2.0f)));
        x *= r / rd;
        y *= r / rd;
    }
...
/**
 * Compute the min and max values of the bounding box of the hull.
 * @param hull The hull containing the object
 * @param color_intrinsics The intrinsic parameters of the camera
 * @return A vector containing (x_max, x_min, y_max, y_min) of the hull
 */
...
inline double get_random_coordinate(double from, double to) {
    double len = to - from;
    long long x = (long long) random() * ((long long) RAND_MAX + 1) + (long long) random();
