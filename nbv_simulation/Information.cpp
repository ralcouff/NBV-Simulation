...
//                bool found_end_point = octo_model->castRay(origin, direction, end_point, true, max_range);
                if (!found_end_point) {
                    ...
                    end_point =
                            ...
                            max_range; // use max range instead of stopping at the unknown
                            // found_endpoint = true;
                }
...
//                                                                       view_space->object_center_world(2) -
//                                                                       view_space->predicted_size) ||
...
//                                                               pcl::PointXYZ(ee(0), ee(1), ee(2)), rr, gg, bb,
//                                                               "line" + to_string(pos) + "-" + to_string(x) +
//                                                               "-" + to_string(
//                                                                       y)); //Add the ray to the set of viewpoints, the first element with the last element key + array + head + tail
//                    first = ray_set->begin();
//                    last = ray_set->end();
//                    stop = last;
//                    Ray *ray = new Ray(*first, *last, ray_set, first, stop);
//                    Ray *ray = new Ray(key_origin, *last, ray_set, first, stop);
